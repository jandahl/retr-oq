...
- **Control-menu box:** flat **white** fill + **1px black** outline (no `box-shadow` bevel — min/max keep `--bevel-out`). Centered black horizontal dash. **Larger** on top-level `.win-sysmenu` (16×14 with the 18px caption); **smaller** on shorter MDI child captions (`.mdi-child .title-bar` / `.mdi-title-bar` ≈ 15px; `.mdi-child .mdi-sysmenu` 10×10; MDI min/max 12×11).
